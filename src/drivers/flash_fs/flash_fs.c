...
/** @brief Converts LittleFS error types to driver defined error types. */
static d_fs_err_t d__lfs_err_to_fs_err(int lfs_rc)
{
    switch (lfs_rc) {
    case LFS_ERR_OK:
...
/** @brief Converts LittleFS entity type to driver defined types. */
static d_fs_type_t d__lfs_type_to_fs_type(uint8_t lfs_type)
{
    switch (lfs_type) {
    case LFS_TYPE_DIR:
...
/** @brief Converts LittleFS info struct to driver info struct. */
static void d__lfs_info_to_fs_info(const struct lfs_info *raw, d_fs_info_t *info)
{
...
/** @brief Converts driver attribute flags to LittleFS attribute flags. */
static int d__fs_flags_to_lfs(d_fs_flags_t flags)
{
...
/** @brief Converts driver whence type to LittleFS whence type. */
static int d__fs_whence_to_lfs(d_fs_whence_t whence)
{
    switch (whence) {
    case D_FS_SEEK_SET:
